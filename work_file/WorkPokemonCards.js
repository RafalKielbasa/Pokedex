...
const PokemonCard = ({ pokemon, setEditedPokemons }) => {
  ...
  const saveAsNewPokemon = (data) => {
    setShowEditFields(false);
    const newPokemon = {
      ...editablePokemon,
      id:
        editablePokemon.length > 0
          ? Math.max(...editablePokemon.map((poke) => poke.id)) + 1
          : pokemon.id + 1,
      stats: [
    ...
    setEditedPokemons((prev) => {
      ...
      localStorage.setItem("editedPokemons", JSON.stringify(updatedPokemons));
  ...
  const onSubmit = (data) => {
    setShowEditFields(false);
    if (buttonName === "saveAsNew") {
      saveAsNewPokemon(data);
    }
          ...
          {showEditFields && (
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <Button
                ...
                name="saveAsNew"
                type="submit"
                onClick={() => setButtonName("saveAsNew")}
              >
