...
const AntPathMatcher__default = /*#__PURE__*/_interopDefaultLegacy(AntPathMatcher);
...
function isDirExists(s) {
  try {
    return fs__default.statSync(s).isDirectory();
  } catch (err) {
    if (err.code === "ENOENT") {
      return false;
    }
...
function isFileExists(s) {
  try {
    return fs__default.statSync(s).isFile();
  } catch (err) {
    if (err.code === "ENOENT") {
      return false;
    }
...
function sendFileContent(res, filePath, mime, logger) {
  const msg = [filePath];
  const data = fs__default.readFileSync(filePath, "utf-8");
...
function sendError(res, msg, logger, statusCode) {
  ...
  return sendData(
    res,
    { message: `${http__default.STATUS_CODES[statusCode]}${customMsg}` },
    [`${statusCode} ${http__default.STATUS_CODES[statusCode]}`, ...Array.isArray(msg) ? msg : [msg]],
    logger,
    statusCode
  );
}
...
const getFilePath = (pathname, mime) => {
  if (isFileExists(pathname)) {
    const { ext } = path__default.parse(pathname);
    return getMime(ext.substring(1)) === mime ? pathname : false;
  }
  let name;
  if (isDirExists(pathname)) {
    name = path__default.join(pathname, "index");
  } else {
    name = pathname;
  }
  name += ".";
...
const parsePathname = (dataRoot, purePath) => {
  let pathname = path__default.join(dataRoot, purePath);
  ...
  if (filePath === "") {
    ...
    pathname = path__default.join(dataRoot, resourceName);
...
function compPropertiesOfDeep(sortBy) {
  function compareByProperty(arg) {
    ...
    return function(a, b) {
      const val_a = _get__default(a, key);
      if (val_a === void 0 || val_a === null) {
        return 0;
      }
      const val_b = _get__default(b, key);
...
const searchPropDeep = (item, q, params) => {
  for (const param of params) {
    ...
    const propValue = _get__default(item, prop);
...
class JsonDb {
  ...
  async load() {
    this.rawContent = await fs__default$1.readFile(this.pathname, { encoding: "utf-8" });
  ...
  async write() {
    await fs__default$1.writeFile(this.pathname, JSON.stringify(this.data), { encoding: "utf-8" });
  }
...
async function onHeadAll(req, res, logger, filePath, defaultLimit) {
  ...
  const q = querystring__default.parse(qs);
...
function getLinks(template, offset, limit, totalCount) {
  ...
  const linkItems = [];
  if (offset !== 0) {
    linkItems.push(util__default.format(template, Math.max(0, offset - limit), "prev"));
  }
  if (offset < totalCount - limit) {
    linkItems.push(util__default.format(template, offset + limit, "next"));
  }
  linkItems.push(util__default.format(template, 0, "first"));
  linkItems.push(util__default.format(template, offset + (Math.ceil((totalCount - offset) / limit) - 1) * limit, "last"));
...
async function onGetAll(req, res, logger, filePath, urlPath, defaultLimit) {
  ...
  if (filterParams) {
    ...
    if (table.count() === 0) {
      return send404(res, [`q=${querystring__default.stringify(q)}`, filePath], logger);
    }
  }
  if (sortParams) {
  ...
  if (offset !== void 0 && limit !== void 0) {
    ...
    delete q.offset;
    const qs2 = querystring__default.stringify(q);
...
function handleOther(req, res, staticRoot, purePath, logger) {
  const pathname = path__default.join(staticRoot, purePath);
  if (!isFileExists(pathname)) {
    return false;
  }
  const ext = path__default.parse(pathname).ext.substring(1);
...
const scanDir = async (dir) => {
  const result = [];
  const items = await fs__default$1.readdir(dir);
  for (const item of items) {
    const pathname = path__default.join(dir, item);
    const stat = await fs__default$1.stat(pathname);
    if (stat.isDirectory()) {
...
async function getJson(mockRoot, staticRoot, options) {
  ...
  const jsonFiles = (await scanDir(mockRoot)).filter(
    (file) => supportedMimes[JSON_MIME_TYPE].some((mime) => mime === path__default.extname(file).substring(1))
  );
  for (const file of jsonFiles) {
    const { name: basename, ext } = path__default.parse(file);
  ...
  for (const file of staticFiles) {
    const ext = path__default.extname(file).substring(1);
    const mime = getMime(ext);
    if (mime) {
...
const matcher = new AntPathMatcher__default();
...
const simpleJsonServerPlugin = (opts = {}) => {
  ...
  return {
    ...
    configResolved(resolvedConfig) {
      ...
      mockRoot = path__default.join(config.root, options.mockDir);
      if (!isDirExists(mockRoot)) {
        logger.warn("Mock directory doesn't exist", mockRoot);
      }
      staticRoot = path__default.join(config.root, options.staticDir);
...
module.exports = simpleJsonServerPlugin;
